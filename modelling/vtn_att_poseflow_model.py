...
class VTN_RGBheat(nn.Module):
    def __init__(self, num_classes=199, num_heads=4, num_layers=2, embed_size=512, sequence_length=16, cnn='rn34',
                 freeze_layers=0, dropout=0, **kwargs):
        super().__init__()
        print("Model: VTNHCPF_Three_View")
        self.heatmap = VTNHCPF(num_classes, num_heads, num_layers, embed_size, sequence_length, cnn, freeze_layers,
                              dropout, **kwargs)
        self.rgb = VTNHCPF(num_classes, num_heads, num_layers, embed_size, sequence_length, cnn, freeze_layers,
                            dropout, **kwargs)
        self.classifier = LinearClassifier(embed_size * 2 * 2, num_classes, dropout)
        self.feature_extractor_heatmap = None
        self.feature_extractor_rgb = None

    def add_backbone(self):
        self.feature_extractor_heatmap = self.heatmap.feature_extractor
        self.feature_extractor_rgb = self.rgb.feature_extractor

    def remove_head_and_backbone(self):
        self.heatmap.classifier = nn.Identity()
    ...
    def freeze(self, layers=2):
        ...
        for i in range(layers):
            for param in self.heatmap.self_attention_decoder.layers[i].parameters():
                param.requires_grad = False
            for param in self.rgb.self_attention_decoder.layers[i].parameters():
                param.requires_grad = False

    def forward_features(self, heatmap=None, rgb=None, pf=None):
